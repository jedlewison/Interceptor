...
public protocol MockURLSessionResponding: class {
    ...
    /// Just before providing a mock response, the `MockURLSession` calls `finalizeMockResponseValuesForRequest:` with a `MockResponseValues` configured to default values.
    ///
    /// This is your opportunity to finalize the response, providing custom data, headers, status code, etc. You do this by setting `MockResponseValues` properties. `MockURLSession` will then use these values to generate the appropriate NSURLResponse and NSData.
...
final public class MockURLSession: NSObject {
    ...
    /// You should never need to use this function. It is an implementation detail. However, you can override it if you want to build your own mechanism for delivering mock url responses. Be careful not to cancel, resume, or suspend the dataTask. Use it only to identify the original request.
